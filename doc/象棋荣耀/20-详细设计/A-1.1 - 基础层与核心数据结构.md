...
- **代码路径**: Assets/Script/_Core/Foundation/
...
- **枚举设计**:
    ...
    - **类型**: [Flags] public enum PieceStatus
        
    - **职责**: 使用二进制位来表示棋子的各种独立状态。多状态组合，所有状态都遵循“有/无”的二元逻辑
    ...
    - **定义**:
        ...
        public enum PieceStatus 
        {
            ...
            // 属性状态 (可组合)
            IsAttackable    = 1 << 8,  // 可被攻击/不可被攻击
            IsAttacking     = 1 << 9,  // 正在攻击/无攻击性
            IsObstacle      = 1 << 10, // 是障碍物/不是障碍物
    ...
    - **使用示例**: 一个在地面移动且具有攻击判定的“车”的状态将是 PieceStatus.Moving | PieceStatus.IsAttackable | PieceStatus.IsAttacking | PieceStatus.IsObstacle。
        
    - **检查状态**: 可以使用位运算 (status & PieceStatus.IsAttacking) != 0 来检查是否处于攻击状态。
...
- **类/结构设计**:
    ...
    1. **PieceData**
        ...
        - **关键成员**:
            ...
            - public PieceStatus status;: 棋子当前的状态。
    ...
    5. **ScriptableObject 配置类**:
        ...
        - **路径**: Assets/Script/_Content/GameConfig/
...
1. **创建ScriptableObject配置**:
    ...
    - Assets/Script/_Content/GameConfig/目录下，右键 Create -> ChessHonor -> Configs -> GameBalanceConfig，创建一个名为DefaultBalance的配置文件。在Inspector中填写最大行动点、棋子移速等数值。
