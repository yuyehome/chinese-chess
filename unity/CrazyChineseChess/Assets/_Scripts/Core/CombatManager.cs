    ...
    public void ProcessCombat(List<PieceComponent> allActivePieces)
    {
        for (int i = 0; i < allActivePieces.Count; i++)
        {
            for (int j = i + 1; j < allActivePieces.Count; j++)
            {
                ...
                {
                    Debug.Log($"[CombatDebug] 检测到碰撞: {pieceA.name} vs {pieceB.name},");
                    Debug.Log($"[CombatDebug] 碰撞前状态 - A: 攻击={pieceA.RTState.IsAttacking}, 无敌={pieceA.RTState.IsVulnerable}, 死亡={pieceA.RTState.IsDead}");
                    Debug.Log($"[CombatDebug] 碰撞前状态 - B: 攻击={pieceB.RTState.IsAttacking}, 无敌={pieceB.RTState.IsVulnerable}, 死亡={pieceB.RTState.IsDead}");
                    ResolveCollision(pieceA, pieceB);
                    Debug.Log($"[CombatDebug] 碰撞后状态 - A: 攻击={pieceA.RTState.IsAttacking}, 无敌={pieceA.RTState.IsVulnerable}, 死亡={pieceA.RTState.IsDead}");
                    Debug.Log($"[CombatDebug] 碰撞后状态 - B: 攻击={pieceB.RTState.IsAttacking}, 无敌={pieceB.RTState.IsVulnerable}, 死亡={pieceB.RTState.IsDead}");
                }
            }
        }
    }
