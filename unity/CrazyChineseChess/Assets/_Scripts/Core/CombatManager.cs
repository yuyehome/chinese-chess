    ...
    public void ProcessCombat(List<PieceComponent> allActivePieces)
    {
        for (int i = 0; i < allActivePieces.Count; i++)
        {
            for (int j = i + 1; j < allActivePieces.Count; j++)
            {
                ...
                {
                    Debug.Log($"[CombatDebug] 检测到碰撞: {a.name} vs {b.name}, 距离: {Mathf.Sqrt(distanceSqr)}");
                    Debug.Log($"[CombatDebug] 碰撞前状态 - A: 攻击={a.RTState.IsAttacking}, 无敌={a.RTState.IsVulnerable}, 死亡={a.RTState.IsDead}");
                    Debug.Log($"[CombatDebug] 碰撞前状态 - B: 攻击={b.RTState.IsAttacking}, 无敌={b.RTState.IsVulnerable}, 死亡={b.RTState.IsDead}");
                    ResolveCollision(pieceA, pieceB);
                    Debug.Log($"[CombatDebug] 碰撞之后的状态 - A: 攻击={a.RTState.IsAttacking}, 无敌={a.RTState.IsVulnerable}, 死亡={a.RTState.IsDead}");
                    Debug.Log($"[CombatDebug] 碰撞之后的状态 - B: 攻击={b.RTState.IsAttacking}, 无敌={b.RTState.IsVulnerable}, 死亡={b.RTState.IsDead}");
                }
            }
        }
    }
